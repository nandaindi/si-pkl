...
@section ('content')
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto">
            <div class="mb-8">
                <h2 class="text-3xl font-bold text-slate-900 font-display">Jadwal Sidang Anda</h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @forelse ($jadwals as $jadwal)
                    <div class="bg-white p-6 rounded-3xl border-2 border-slate-200 transition-colors shadow-sm flex flex-col gap-4 relative overflow-hidden group">
                        
                        <div class="flex items-start justify-between">
                            <div class="flex items-center gap-3">
                                <div class="w-12 h-12 rounded-full bg-purple-50 text-purple-700 flex items-center justify-center font-bold text-lg border border-purple-100 shrink-0">
                                    {{ substr($jadwal->siswa->user->name, 0, 1) }}
                                </div>
                                <div>
                                    <h3 class="font-bold text-slate-900 text-base leading-tight">{{ $jadwal->siswa->user->name }}</h3>
                        ...
                        <div class="bg-slate-50 rounded-2xl p-4 border border-slate-100 mt-2">
                            <div class="flex items-center gap-3 mb-3">
                                <div class="w-8 h-8 rounded-full bg-white flex items-center justify-center shadow-sm text-slate-400 shrink-0">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm font-bold text-slate-700">{{ \Carbon\Carbon::parse($jadwal->waktu)->isoFormat('dddd, D MMM Y') }}</p>
                                </div>
                            </div>
                            <div class="flex items-center gap-3 mb-3">
                                <div class="w-8 h-8 rounded-full bg-white flex items-center justify-center shadow-sm text-slate-400 shrink-0">
                ...
                @empty
                    <div class="col-span-full">
                        <div class="bg-white p-10 rounded-3xl border border-slate-200 text-center flex flex-col items-center justify-center shadow-sm">
