...
@section ('content')
    @section ('header')
        <h2 class="font-semibold text-xl text-slate-800 leading-tight">{{ __('Input Nilai Sidang') }}</h2>
    @endsection
    <div class="py-6">
                            ...
                            @forelse ($jadwals as $jadwal)
                                ...
                                <tr class="hover:bg-slate-50 transition-colors group">
                                    <td class="px-6 py-4">
                                        ...
                                        <div class="flex flex-col gap-0.5 text-xs text-slate-500 mt-1">
                                            <span>NISN: {{ $jadwal->siswa->nisn }}</span>
                                            <span>{{ $jadwal->siswa->kelas }} {{ $jadwal->siswa->jurusan }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="text-sm font-bold text-slate-700">
                                            {{ \Carbon\Carbon::parse($jadwal->waktu)->isoFormat('dddd, D MMM Y') }}
                                        </div>
                                        <div class="flex flex-col gap-0.5 text-xs text-slate-500 mt-1">
                                            <span>Jam: {{ \Carbon\Carbon::parse($jadwal->waktu)->isoFormat('HH:mm') }} WIB</span>
                                            <span>Ruang: {{ $jadwal->ruangan }}</span>
                                        </div>
                                    </td>
                                    <td class="px-3 py-4">
                                        <div
                                    ...
                                    <td class="px-3 py-4">
                                        <div
                                            class="text-center font-bold font-label {{ isset($nilai->nilai_penguji) ? 'text-slate-900 text-base' : 'text-slate-400 text-sm' }}"
                                        >
                                            {{ $nilai->nilai_penguji ?? '-' }}
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        ...
                                        >
                                            ...
                                            <button
                                                type="submit"
                                                class="px-4 py-2 bg-blue-600 hover:bg-slate-900 text-white font-bold text-xs rounded-xl transition-colors shadow-sm"
                                            >
                                                Simpan Nilai
                                            </button>
                                        </form>
