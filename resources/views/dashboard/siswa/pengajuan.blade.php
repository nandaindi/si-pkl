                    ...
                    <div class="bg-white border {{ $pengajuan->status === 'disetujui' ? 'border-green-200 shadow-green-100/50' : ($pengajuan->status === 'ditolak' ? 'border-rose-200' : 'border-slate-200') }} rounded-3xl p-8 min-h-[260px] flex flex-col shadow-sm relative overflow-hidden group hover:shadow-md transition-all">
                        ...
                        <div class="absolute top-8 right-8">
                            ...
                            @elseif ($pengajuan->status === 'ditolak')
                                <span class="px-3 py-1.5 bg-rose-50 text-rose-700 text-[10px] uppercase font-black tracking-widest rounded-lg border border-rose-200/50">Ditolak</span>
                            @else
                        ...
                        <div class="pt-6 border-t border-slate-100 flex items-center justify-between mt-auto">
                            ...
                            @if ($pengajuan->status === 'pending')
                                <form action="{{ route('siswa.pengajuan.destroy', $pengajuan->id) }}" method="POST" data-confirm="Yakin ingin membatalkan pengajuan ini? Anda bisa mengajukan tempat lain setelahnya." class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-[11px] font-bold text-rose-600 hover:text-rose-700 bg-rose-50 hover:bg-rose-100 px-3.5 py-1.5 rounded-lg transition-colors flex items-center gap-1.5">
