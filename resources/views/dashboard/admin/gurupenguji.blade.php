    ...
    <div>
        <div class="max-w-7xl mx-auto">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4">
                <div>
                    <h2 class="text-2xl font-bold text-slate-900">Kelola Guru Penguji</h2>
                    <p class="text-slate-500 text-sm mt-1">Manajemen guru penguji untuk sidang PKL.</p>
                </div>
                <a
                    href="{{ route('admin.guru-penguji.create') }}"
            ...
            <form action="{{ route('admin.guru-penguji.index') }}" method="GET" class="relative w-96 mb-6">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari NIP atau nama penguji..." class="w-full pl-10 pr-4 py-2 border border-slate-100 rounded-xl focus:border-slate-300 text-sm bg-white shadow-sm" />
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            ...
                            >
                                ...
                                <th class="px-6 py-4 font-semibold whitespace-nowrap">Nama Penguji</th>
                        ...
                        <tbody class="divide-y divide-slate-200">
                            @forelse ($gurus as $guru)
                                <tr class="hover:bg-slate-50 transition-colors">
                                    <td class="px-6 py-4 text-sm text-slate-900 font-label">{{ $guru->nip ?? '-' }}</td>
                                    ...
                                    <td class="px-6 py-4 text-sm text-center">
                                        <div class="flex items-center justify-center gap-2">
                                            <a
                                                href="{{ route('admin.guru-penguji.edit', $guru->id) }}"
                                            ...
                                            <form
                                                action="{{ route('admin.guru-penguji.destroy', $guru->id) }}"
                                                method="POST"
                                                data-confirm="Yakin ingin menghapus guru penguji ini?"
                                                class="inline"
                                            >
                                                @csrf
                                                @method ('DELETE')
                                ...
                                <tr>
                                    <td colspan="4" class="px-6 py-12 text-center text-slate-500">
                                        <div class="flex flex-col items-center justify-center">
                                            <p class="text-sm font-medium">Belum ada data guru penguji sidang.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
